...
export default function ReviewsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FAFBFD] to-[#FFFFFF] font-sans pb-16 relative overflow-x-hidden">
      {/* Background glow effects */}
      <div className="absolute top-0 left-0 w-[200px] h-[200px] bg-[#30C45D]/5 rounded-full filter blur-3xl pointer-events-none"></div>
      
      <div className="w-full max-w-md md:max-w-5xl mx-auto px-4 md:px-6 pt-2 md:pt-0">
        
        {/* Header */}
        <header className="flex items-center justify-between py-2 relative z-10 w-full mb-4 md:mb-6">
          <button 
          ...
          <div className="w-10 h-10 rounded-full bg-transparent" aria-hidden="true" />
        </header>

        {/* Two Column Grid */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-start w-full relative z-10">
          
          {/* Left Column: Rating breakdown */}
          <div className="col-span-1 md:col-span-5 flex flex-col gap-4">
            {/* Hero Breakdown Card */}
            <div className="bg-white rounded-[32px] p-6 shadow-[0_15px_35px_-5px_rgba(0,0,0,0.015),_0_5px_15px_rgba(0,0,0,0.005)] border border-slate-100/80 flex flex-col items-center">
              {/* Large Green Rating */}
          ...
          {/* Right Column: Filters and reviews list */}
          <div className="col-span-1 md:col-span-7 flex flex-col gap-4">
            
            {/* Filters Pills */}
            <div className="flex gap-2 pb-2 overflow-x-auto md:overflow-x-visible md:flex-wrap scrollbar-hide -mx-4 md:-mx-0 px-4 md:px-0 select-none touch-pan-x min-h-[48px]">
              {filters.map((filter) => {
